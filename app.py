...
with st.sidebar:
    ...
    input_type = st.radio("Choose Input Method", ["YouTube URL / Path", "Upload File"], horizontal=True)
    ...
    if input_type == "YouTube URL / Path":
        ...
        uploaded_file = st.file_uploader("Upload Audio/Video", type=["mp3", "wav", "m4a", "mp4", "mpeg"])
    ...
    if not is_valid:
        ...
        try:
            ...
            if input_type == "Upload File" and uploaded_file is not None:
                ...
                temp_file_path = os.path.join("downloades", clean_name)
                with open(temp_file_path, "wb") as f:
                    f.write(uploaded_file.getbuffer())
                source_to_process = temp_file_path
            else:
                ...
                for c in chunks:
                    if os.path.exists(c):
                        try:
                            ...
                            pass
                try:
                    wav_path = chunks[0].split("_chunk_")[0]
